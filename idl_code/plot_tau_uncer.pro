...
pro plot_tau_uncer
	...
	plot_isolated=0 ; get the spatial distribution of tauerr
	plot_clrcld=0 ; get the relationship of aerosol tauerr vs tau
    plot_cldtauvsaerotau=1
...
;	dir='/u/sciteam/yulanh/mydata/radar-lidar_out/CAL_ACLAY/Parallel_1/aerosol_cloud_overlap/'
    ...
    for fi=0,Nf-1 do begin
		...
		IF (strlen(fname) eq 86) Then Begin
			...
			read_dardar,fname,'aero_only_tauvsuncer',aeroonly_tauvsuncer
			read_dardar,fname,'pdf_aero_only_tau',aeroonly_pdf

			Taeroonly_tau0_numh=Taeroonly_tau0_numh+aeroonly_negativetau_numh
			...
			Taeroonly_pdf=Taeroonly_pdf+aeroonly_pdf
			ind=where(aeroonly_pdf eq 0)
			aeroonly_tauvsuncer[ind]=0.0
			Taeroonly_tauvsuncer=Taeroonly_tauvsuncer+aeroonly_tauvsuncer

			read_dardar,fname,'isolate_aerosol_num',aero_isolate_numh
			read_dardar,fname,'isolate_aerosol_negativetau_num',aeroisolate_negativetau_numh
			read_dardar,fname,'isolate_aerosol_tau_uncer',aeroisolate_tauuncer_spatial
			read_dardar,fname,'isolate_aero_tauvsuncer',aeroisolate_tauvsuncer
			read_dardar,fname,'pdf_isolate_aero_tau',aeroisolate_pdf
			read_dardar,fname,'isolate_cldtau_vsaerotau',isolate_cldtau
			read_dardar,fname,'isolate_cldtauerr_vsaerotau',isolate_cldtauerr

			Taero_isolate_numh= Taero_isolate_numh + aero_isolate_numh
			Taero_isolate_tau0_numh= Taero_isolate_tau0_numh + aeroisolate_negativetau_numh
			...
			Taeroisolate_tauerr_spa=Taeroisolate_tauerr_spa + aeroisolate_tauuncer_spatial*$
				(aero_isolate_numh)
			Taeroisolate_pdf=Taeroisolate_pdf+aeroisolate_pdf
			ind=where(aeroisolate_pdf eq 0)
			aeroisolate_tauvsuncer[ind]=0.0
			Taeroisolate_tauvsuncer=Taeroisolate_tauvsuncer+aeroisolate_tauvsuncer
	
			Tisolate_cldtau = Tisolate_cldtau + isolate_cldtau
			Tisolate_cldtauerr = Tisolate_cldtauerr + isolate_cldtauerr		

			read_dardar,fname,'incloud_aerosol_num',aero_incld_numh
 			read_dardar,fname,'incloud_aerosol_negativetau_num',aeroincld_negativetau_numh
			read_dardar,fname,'incloud_aerosol_tau_uncer',aeroincld_tauuncer_spatial
			read_dardar,fname,'incld_aero_tauvsuncer',aeroincld_tauvsuncer
			read_dardar,fname,'pdf_incld_aero_tau',aeroincld_pdf
			read_dardar,fname,'incld_cldtau_vsaerotau',incld_cldtau
			read_dardar,fname,'incld_cldtauerr_vsaerotau',incld_cldtauerr

			Taero_incld_numh = Taero_incld_numh + aero_incld_numh
			Taero_incld_tau0_numh = Taero_incld_tau0_numh + aeroincld_negativetau_numh
			...
			Taeroincld_tauerr_spa=Taeroincld_tauerr_spa + aeroincld_tauuncer_spatial*$
				(aero_incld_numh)
			Taeroincld_pdf=Taeroincld_pdf+aeroincld_pdf
			ind=where(aeroincld_pdf eq 0)
			aeroincld_tauvsuncer[ind]=0.0
			Taeroincld_tauvsuncer=Taeroincld_tauvsuncer+aeroincld_tauvsuncer
	
			Tincld_cldtau = Tincld_cldtau + incld_cldtau
			Tincld_cldtauerr = Tincld_cldtauerr + incld_cldtauerr		

		EndIf
	endfor

	;==== to plot cloud tau vs aerosol tau relationship ======
...
;	c3.scale,5,4
	...
	IF (plot_isolated eq 1) Then Begin
      ...
      maxvalue1=1.0
 	  ...
 	  alltauerr=total(total(Taeroisolate_tauerr_spa,3),3)/total(total(Taero_isolate_numh-Taero_isolate_tau0_numh,3),3)
	  alltauerr1=total(weights*total(alltauerr,1,/nan))/(dimx*total(weights))
	  print,'total isolated tau err',alltauerr1

		tauerr=total(reform(Taeroisolate_tauerr_spa[*,*,0,*]+Taeroisolate_tauerr_spa[*,*,3,*]+Taeroisolate_tauerr_spa[*,*,7,*]),3)/$
        total(reform(Taero_isolate_numh[*,*,0,*]+Taero_isolate_numh[*,*,3,*]+Taero_isolate_numh[*,*,7,*]-$
		...
		tauerr2=total(reform(Taeroisolate_tauerr_spa[*,*,1,*]+Taeroisolate_tauerr_spa[*,*,4,*]+Taeroisolate_tauerr_spa[*,*,6,*]),3)/$
        total(reform(Taero_isolate_numh[*,*,1,*]+Taero_isolate_numh[*,*,4,*]+Taero_isolate_numh[*,*,6,*]-$
		...
		tauerr3=total(reform(Taeroisolate_tauerr_spa[*,*,2,*]+Taeroisolate_tauerr_spa[*,*,5,*]+Taeroisolate_tauerr_spa[*,*,8,*]),3)/$
        total(reform(Taero_isolate_numh[*,*,2,*]+Taero_isolate_numh[*,*,5,*]+Taero_isolate_numh[*,*,8,*]-$
		...
		;=== for merged layer clouds
 	  	...
 	  	alltauerr=total(total(Taeroincld_tauerr_spa,3),3)/total(total(Taero_incld_numh-Taero_incld_tau0_numh,3),3)
	  	alltauerr1=total(weights*total(alltauerr,1,/nan))/(dimx*total(weights))
	  	print,'total incld tau err',alltauerr1

		tauerrm0=total(reform(Taeroincld_tauerr_spa[*,*,0,*]),3)/$
        total(reform(Taero_incld_numh[*,*,0,*]-Taero_incld_tau0_numh[*,*,0,*]),3)
		...
		tauerrm1=total(reform(Taeroincld_tauerr_spa[*,*,1,*]),3)/$
        total(reform(Taero_incld_numh[*,*,1,*]-Taero_incld_tau0_numh[*,*,1,*]),3)
		...
		tauerrm2=total(reform(Taeroincld_tauerr_spa[*,*,2,*]+Taeroincld_tauerr_spa[*,*,3,*]),3)/$
        total(reform(Taero_incld_numh[*,*,2,*]+Taero_incld_numh[*,*,3,*]-$
		...
		tauerrm3=total(reform(Taeroincld_tauerr_spa[*,*,4,*]+Taeroincld_tauerr_spa[*,*,5,*]),3)/$
        total(reform(Taero_incld_numh[*,*,4,*]+Taero_incld_numh[*,*,5,*]-$
			Taero_incld_tau0_numh[*,*,4,*]-Taero_incld_tau0_numh[*,*,5,*]),3)
	...
	c.save,'aerosol_tau_uncertainty_spacad50.png'
	stop
	EndIf
