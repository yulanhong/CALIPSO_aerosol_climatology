...
pro  plot_annual_aerocld_property2

   	fname=file_search('/u/sciteam/yulanh/scratch/CALIOP/output','*_night.hdf')
    ...
    for fi=0,Nf-1 do begin
		IF (strlen(fname[fi]) eq 97 or strlen(fname[fi]) eq 98)  Then Begin 
	;86 for all, 95 for day_land, 97 for land_night, 96 for ocean_day, 98 for ocean_night,112 for land_daynight,113 for ocean_daynight
		print,fname[fi]
	...
	allaero_num=total(Taeroincld_num,3) + total(Taeroisolate_num,3) + Taeroonly_num
	...
	print,'ave aeroonly fraction',total(Taeroonly_num)/float(total(allaero_num))
...
;	print,'avetau_aerocld tau, weight average',total(weights*total(avetau_aerocld,1,/nan))/(dimx*total(weights))
	...
	print,'ave aerocld num fraction',(total(Taeroincld_num)+total(Taeroisolate_num))/float(total(allaero_num))
...
;	print,'avetau_aero isolated tau, weight average',total(weights*total(avetau_aerois,1,/nan))/(dimx*total(weights))
	...
	print,'ave aero isolated fraction',total(Taeroisolate_num)/float(total(allaero_num))
...
;	print,'avetau_aero incld tau, weight average',total(weights*total(avetau_aeroin,1,/nan))/(dimx*total(weights))
	...
	print,'ave aero incld fraction',total(Taeroincld_num)/float(total(allaero_num))
   ...
   stop
	...
	IF (plotfre_merged eq 1) Then Begin
		...
		tauerr=['0.047','0.075','0.045','0.078'] ; check update from plot_tau_uncer2.pro
		...
		for i=0,Nmerged-1 do begin
			freh=(reform(data[*,*,i]))/float(allaero_num)
...
;		data1=reform(data[*,*,i])
		...
		c=image(data1,lon,lat,dim=[570,650],rgb_table=33,min_value=minvalue,max_value=maxvalue,$
			 ...
			 c.scale,1.0,1
			 pos=c.position
			 t1=text(pos[0],pos[3]+0.01,'(a1) $ave f$='+freh1+'%'+'',font_size=fontsz)
			...
			data1=(reform(data[*,*,1]))/float(allaero_num)
;		data1=reform(data[*,*,i])
			ind=where(finite(data1) eq 0)
			data1[ind]=0.0
			freh1=strcompress(freh_ave[1]*100,/rem)	
			freh1=strmid(freh1,0,4)	
			c=image(data1,lon,lat,rgb_table=33,min_value=minvalue,max_value=maxvalue,$
			 ...
			 mc['Longitudes'].label_angle=0
			 pos=c.position
			 t1=text(pos[0],pos[3]+0.01,'(a2) ave f='+freh1+'%'+'',font_size=fontsz)
			...
			data3=(reform(data[*,*,2])+reform(data[*,*,3]))/float(allaero_num)
			ind=where(finite(data3) eq 0)
			data3[ind]=0.0
			freh=total(data3*allaero_num)/total(float(allaero_num))
			freh1=strcompress(freh*100,/rem)	
			freh1=strmid(freh1,0,4)	
			c2=image(data3,lon,lat,rgb_table=33,min_value=minvalue,max_value=maxvalue,$
			 ...
			 c2.scale,1.0,1
			 pos=c2.position
			 t3=text(pos[0],pos[3]+0.01,'(a3) ave f='+freh1+'%'+'',font_size=fontsz)
			...
			data5=(reform(data[*,*,4])+reform(data[*,*,5]))/float(allaero_num)
			ind=where(finite(data5) eq 0)
			data5[ind]=0.0
			freh=total(data5*allaero_num)/total(float(allaero_num))
			freh1=strcompress(freh*100,/rem)	
			freh1=strmid(freh1,0,4)	
			c5=image(data5,lon,lat,rgb_table=33,min_value=minvalue,max_value=maxvalue,$
			 ...
			 c5.scale,1.0,1
			 pos=c5.position
			 t5=text(pos[0],pos[3]+0.01,'(a4) ave f='+freh1+'%'+'',font_size=fontsz)
			...
			c.save,'aerosol_cldmerged_fre_aod_cad20_screened_fraction.png'
	...
	IF (plotfre_isolated eq 1) Then Begin
		...
		tauerr=['0.078','0.038','0.076'] ; from  plot_tau_uncer.pro

		for i=0,Nisolate-1 do begin
			freh=total(reform(data[*,*,i]))/total(float(allaero_num))
            ind=where(finite(freh) eq 0)
            freh[ind]=0.0
            freh1=total(weights*total(freh,1,/nan))/(dimx*total(weights))
			;tau=reform(Taeroisolate_tau[*,*,i])/reform(Taeroisolate_num[*,*,i])
			tau1=total(Taeroisolate_tau[*,*,i])/float(total(data[*,*,i]))
			print,'isolated weighted freh and tau',freh1,tau1
		endfor		
		;== to plot=== 
		data1=(reform(data[*,*,0])+reform(data[*,*,3])+reform(data[*,*,7]))/float(allaero_num)
        ind=where(finite(data1) eq 0)
        data1[ind]=0.0
        ;freh=total(weights*total(data1,1,/nan))/(dimx*total(weights))
		freh=total(data1*allaero_num)/float(total(allaero_num))
	...
	;	tau1=total(weights*total(tau,1,/nan))/(dimx*total(weights))
		...
		data2=(reform(data[*,*,1])+reform(data[*,*,4])+reform(data[*,*,6]))/float(allaero_num)
        ind=where(finite(data2) eq 0)
        data2[ind]=0.0
        freh=total((data2*allaero_num))/total(allaero_num)
		...
		data3=(reform(data[*,*,2])+reform(data[*,*,5])+reform(data[*,*,8]))/float(allaero_num)
        ind=where(finite(data3) eq 0)
        data3[ind]=0.0
        ;freh=total(weights*total(data3,1,/nan))/(dimx*total(weights))
		freh=total(data3*allaero_num)/float(total(allaero_num))
        print,'other cloud aerosol weighted freh',freh
        freh1=strcompress(freh*100,/rem)
        freh1=strmid(freh1,0,4)
        c4=image(data3,lon,lat,rgb_table=33,min_value=minvalue,max_value=maxvalue,$
             position=pos5,font_size=fontsz,/current)
             mp=map('Geographic',limit=maplimit,transparency=30,overplot=c4)
             grid=mp.MAPGRID
             grid.label_position=0
             ...
             c4.scale,1.0,1
             pos=c4.position
             t1=text(pos[0],pos[3]+0.01,'(a3) ave f='+freh1+'%'+' ',font_size=fontsz)
...
;		tau1=total(weights*total(tau,1,/nan))/(dimx*total(weights))
		...
		c.save,'aerosol_cldisolate_fre_aod_cad20_screened_fraction.png'	
