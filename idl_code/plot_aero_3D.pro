...
pro plot_aero_3D
   ...
   print,systime()
   month='Mar'  
   lidfname=file_search('/u/sciteam/yulanh/mydata/radar-lidar_out/CAL_ACLAY/Parallel_1','*03.hdf')
...
;   maxlon=180	
   ...
   for i=0,Nf-1 do begin
	...
	IF strlen(lidfname[i]) eq 94 Then Begin 
...
stop
vol=volume(data,hints=3,rgb_table0=33,axis_style=2,/auto_render,max_value=1.0,min_value=0.2,$
	volume_location=[lon1[0],lat1[0],0],volume_dimensions=[(max(lon1)-min(lon1)),(max(lat1)-min(lat1)),dimh],$
        ztickvalues=ztickv,ztickname=zname,ytitle='Latitude',$
	xtitle='Longitude',ztitle='Altitude (km)',zrange=[0,30])

ave_aero=reform(ave_aero_only_tau[*,*,0])
im=image(aero_freh,lon1,lat1,max_value=1.0,min_value=0.2,rgb_table=33,/current,overplot=vol)
...
;mp=map('Geographic',limit=maplimit,overplot=im,transparency=30)
;grid=mp.MAPGRID
;grid.label_position=0
;grid.linestyle='dotted'
;mc=mapcontinents(/continents,transparency=30)
;vol.save,'aerosol_3D_probability_'+month+'.png'
