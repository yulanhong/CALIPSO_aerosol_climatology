...
pro  plot_annual_aerocld_mergedaynight_property2

;	fname=file_search('/u/sciteam/yulanh/mydata/radar-lidar_out/CAL_ACLAY/Parallel_1/aerosol_cloud_overlap_screen_2/','*ocean*')
 	lidfname=file_search('/u/sciteam/yulanh/scratch/CALIOP/output','*land_day.hdf')
    ...
    for fi=0,Nf-1 do begin
		IF (strlen(fname[fi]) le 151) Then Begin
		print,fname[fi]
    ...
    avetau_aerocld=$
	(total(Taeroincld_tau+Taeroisolate_tau))$
	/(total((Taeroincld_num+Taeroisolate_num)))
...
;	print,'avetau_aero incld tau, weight average',total(weights*total(avetau_aeroin,1,/nan))/(dimx*total(weights))
	...
	IF (plotfre_merged eq 1) Then Begin
		...
		tauerr=['0.047','0.077','0.043','0.077'] ; check update from plot_tau_uncer2.pro
		...
		for i=0,Nmerged-1 do begin
			...
			freh_ave[i]=total(data[*,*,i])/float(total(Tobsnum))
...
;		data1=reform(data[*,*,i])
		...
		c=image(data1,lon,lat,dim=[570,650],rgb_table=33,min_value=minvalue,max_value=maxvalue,$
			 ...
			 c.scale,1.0,1
			 pos=c.position
			 t1=text(pos[0],pos[3]+0.01,'a1) $\bar f_a$='+freh1+'%'+'   scale=1.0',font_size=fontsz)
...
;		data1=reform(data[*,*,i])
			...
			data3[ind]=0.0
			freh=total(data3*Tobsnum)/total(float(Tobsnum))
			...
			data5[ind]=0.0
			freh=total(data5*Tobsnum)/total(float(Tobsnum))
			freh1=strcompress(freh*100,/rem)	
			freh1=strmid(freh1,0,4)	
...
;			c.save,'aerosol_cldmerged_fre_aod_cad20_screened_ocean_land.png'
	...
	IF (plotfre_isolated eq 1) Then Begin
		...
		tauerr=['0.084','0.038','0.073'] ; from  plot_tau_uncer.pro

		for i=0,Nisolate-1 do begin
			freh1=total(reform(data[*,*,i]))/total(float(Tobsnum))
            ind=where(finite(freh) eq 0)
            freh[ind]=0.0
            ;freh1=total(weights*total(freh,1,/nan))/(dimx*total(weights))
		...
		freh=total(data1*Tobsnum)/float(total(Tobsnum))
	...
	;	tau1=total(weights*total(tau,1,/nan))/(dimx*total(weights))
        ...
        data2[ind]=0.0
        freh=total((data2*Tobsnum))/total(Tobsnum)
		...
		freh=total(data3*Tobsnum)/float(total(Tobsnum))
        ...
        c4=image(data3*10,lon,lat,rgb_table=33,min_value=minvalue,max_value=maxvalue,$
             ...
             t1=text(pos[0]-0.055,pos[1]+0.02,'Other isolate layers',orientation=90,font_size=fontsz)
...
;	xtitle='Backscatter'
	...
	res=value_locate(ydata1_cdf,50.0)
	print,'cloud-free aerosol median', 10.0^xdata[res]
	...
	res=value_locate(ydata2_cdf,50.0)
	print,'below cloud aerosol median', 10.0^xdata[res]
	...
	res=value_locate(ydata3_cdf,50.0)
	print,'above cloud aerosol median', 10.0^xdata[res]
	...
	res=value_locate(ydata4_cdf,50.0)
	print,'other cloud aerosol median', 10.0^xdata[res]
	p3=plot(xdata,ydata4,color='cyan',xrange=xrange,yrange=yrange,thick=lnthk,$
		overplot=po2,name='Other aerosol layers')
	...
	res=value_locate(ydata5_cdf,50.0)
	print,'merged layer only median', 10.0^xdata[res]
	...
	res=value_locate(ydata6_cdf,50.0)
	print,'merged layer below cld median', 10.0^xdata[res]
	...
	res=value_locate(ydata7_cdf,50.0)
	print,'merged layer above cld median', 10.0^xdata[res]
	...
	res=value_locate(ydata8_cdf,50.0)
	print,'merged layer other  median', 10.0^xdata[res]
	i4=plot(xdata,ydata8,color='cyan',xrange=xrange,yrange=yrange,thick=lnthk,$
		overplot=po1,name='Other merged layers')

	ld=legend(target=[po1,i1,i2,i3,i4],position=[pos1[0]+0.02,pos1[3]-0.01],transparency=100,horizontal_alignment=0,$
		font_size=fontsz,vertical_spacing=0.01)
	
;	  po1.save,'taupdf_aerocld_merged_isolated_combine_cad20_screened.png'
	stop
   EndIf ;end plotpdf
